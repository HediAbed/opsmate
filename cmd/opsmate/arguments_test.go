...
func TestParseArgumentsRejectsInvalidInput(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if code := failure.CodeOf(err); code != failure.CodeInvalidArgument {
				t.Fatalf("failure code = %q, want %q", code, failure.CodeInvalidArgument)
			}
		})
	}
}

func TestArgumentErrorHandlesMissingDetail(t *testing.T) {
	var nilError *argumentError
	for _, err := range []*argumentError{nilError, {}} {
		if got := err.Error(); got != "invalid command-line arguments" {
			t.Fatalf("Error() = %q, want generic argument error", got)
		}
	}
}
