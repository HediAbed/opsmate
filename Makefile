...
build: version-check ## Build the host binary.
	...
	CGO_ENABLED=0 go build $(BUILD_FLAGS) -ldflags='$(LINK_FLAGS)' -o "$$temporary_binary" $(COMMAND); \
...
test-windows: ## Compile every test package for Windows.
	...
	for package in $$(go list ./...); do \
		...
		CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go test -c \
			-o "$$output_directory/package-$$package_number.test.exe" "$$package"; \
	done
...
repository-check: version-check ## Validate repository metadata and file modes.
	...
	@forbidden_glue=-; \
	forbidden_pattern="co$${forbidden_glue}authored$${forbidden_glue}by|generated[ $${forbidden_glue}]by|machine$${forbidden_glue}generated|ai$${forbidden_glue}generated"; \
	set +e; \
...
$(1)-$(2): version-check | $(DIST_DIR)
	CGO_ENABLED=0 GOOS=$(1) GOARCH=$(2) go build $(BUILD_FLAGS) -ldflags='$(LINK_FLAGS)' -o "$(DIST_DIR)/$(BINARY)-$(1)-$(2)$(if $(filter windows,$(1)),.exe,)" $(COMMAND)
endef
