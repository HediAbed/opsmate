...
func projectSecrets(items []kube.ResourceMetadata, now time.Time) []cluster.Secret {
	return projectSlice(items, now, func(item kube.ResourceMetadata, now time.Time) cluster.Secret {
...
func projectCRDInstances(items []kube.ResourceMetadata, now time.Time) []cluster.CRDInstance {
	return projectSlice(items, now, func(item kube.ResourceMetadata, now time.Time) cluster.CRDInstance {
		return cluster.CRDInstance{
			Name:      item.Name,
			Namespace: item.Namespace,
			Age:       projectedAge(now, item.CreatedAt),
		}
...
func projectReplicaSets(items []appsv1.ReplicaSet, now time.Time) []cluster.ReplicaSet {
	return projectSlice(items, now, func(item appsv1.ReplicaSet, now time.Time) cluster.ReplicaSet {
		desired := int32(0)
...
func projectRBAC(resources kube.RBACResources, now time.Time) []cluster.RBAC {
	...
	items := make([]cluster.RBAC, 0, capacity)
...
func projectedRBACRow(kind string, object metav1.Object, count int, scope string, now time.Time) cluster.RBAC {
	return cluster.RBAC{
		Kind:      kind,
		Name:      object.GetName(),
...
func projectCRDs(items []apiextensionsv1.CustomResourceDefinition, now time.Time) []cluster.CRD {
	return projectSlice(items, now, func(item apiextensionsv1.CustomResourceDefinition, now time.Time) cluster.CRD {
