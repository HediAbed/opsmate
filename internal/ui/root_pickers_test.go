...
func TestRootHandleCtxPickerMouse_WheelDownAdvancesCursor(t *testing.T) {
	...
	m.contexts = []cluster.KubeContext{{Name: "a"}, {Name: "b"}, {Name: "c"}}
	m.ctxCursor = 0
...
func TestRootRenderContent_AnalysisVisibleSplitsLayout(t *testing.T) {
	m := freshRoot(t)
	m.analysisPanel.SetVisible(true)
	out := m.renderContent()
	if out == "" {
		t.Error("renderContent with analysis panel visible should still produce output")
	}
}
