...
func projectStatefulSets(items []appsv1.StatefulSet, now time.Time) []cluster.StatefulSet {
	return projectSlice(items, now, func(item appsv1.StatefulSet, now time.Time) cluster.StatefulSet {
		return cluster.StatefulSet{
			Name:      item.Name,
...
func projectDaemonSets(items []appsv1.DaemonSet, now time.Time) []cluster.DaemonSet {
	return projectSlice(items, now, func(item appsv1.DaemonSet, now time.Time) cluster.DaemonSet {
...
func projectConfigMaps(items []corev1.ConfigMap, now time.Time) []cluster.ConfigMap {
	return projectSlice(items, now, func(item corev1.ConfigMap, now time.Time) cluster.ConfigMap {
...
func projectNodes(items []corev1.Node, now time.Time) []cluster.Node {
	return projectSlice(items, now, projectNode)
}

func projectNode(item corev1.Node, now time.Time) cluster.Node {
...
func projectJobs(items []batchv1.Job, now time.Time) []cluster.Job {
	return projectSlice(items, now, projectJob)
}

func projectJob(item batchv1.Job, now time.Time) cluster.Job {
	completions := int32(1)
