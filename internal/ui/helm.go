...
func NewHelmModel(namespace string, commands helmCommands) HelmModel {
...
func (m *HelmModel) SetSize(width, height int) {
	...
	m.releaseTable.SetColumns(computeColumns(width, helmColSpecs))
...
func (m *HelmModel) applyHelmReleases(msg helmReleasesMsg) {
	m.loading = false
	if msg.Err != nil {
		m.err = msg.Err
		m.releases = nil
...
func (m HelmModel) applyHelmValues(msg helmValuesMsg) HelmModel {
	if !m.valuesPopupVisible {
		return m
	}
