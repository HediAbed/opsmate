...
func (m DashboardModel) handleSupervisedLiveMessage(message supervisedLiveMsg) (DashboardModel, tea.Cmd) {
	switch {
...
func (m DashboardModel) ownsSupervisedLiveMessage(message supervisedLiveMsg) bool {
...
func (m DashboardModel) handlePodLivePayload(payload tea.Msg) (DashboardModel, tea.Cmd) {
	message, ok := payload.(liveSnapshotMsg[cluster.Pod])
	if !ok {
		return m, nil
	}
...
func (m DashboardModel) handleDeploymentLivePayload(payload tea.Msg) (DashboardModel, tea.Cmd) {
	message, ok := payload.(liveSnapshotMsg[cluster.Deployment])
	if !ok {
		return m, nil
	}
...
func (m DashboardModel) handleEventLivePayload(payload tea.Msg) (DashboardModel, tea.Cmd) {
	message, ok := payload.(liveSnapshotMsg[cluster.Event])
	if !ok {
		return m, nil
	}
...
func (m DashboardModel) handleDashboardLiveSetClosed(kind dashboardDataKind) (DashboardModel, tea.Cmd) {
	...
	case dashboardPods:
		m.podLive.Stop()
		m.podLiveError = ErrLiveUpdatesStopped
		m.loading = false
	case dashboardDeployments:
		m.deploymentLive.Stop()
		m.deploymentLiveError = ErrLiveUpdatesStopped
	case dashboardEvents:
		m.eventLive.Stop()
		m.eventLiveError = ErrLiveUpdatesStopped
