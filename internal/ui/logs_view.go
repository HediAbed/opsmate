...
func (m LogsModel) renderExplainPanel() string {
	...
	if m.lineExplanationLoading {
		...
		content = theme.Error.Render("Analysis error: " + sanitizeTerminalLine(m.lineExplanationErr.Error()))
...
func (m LogsModel) renderPodPopupOverlay(width, height int) string {
	...
	for index := start; index < end; index++ {
		...
		} else if pod.Name == m.selectedPod && namespacesMatch(m.selectedPodNS(), pod.Namespace) {
