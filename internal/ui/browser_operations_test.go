...
func TestRootDrillDownUsesTypedInspectionReference(t *testing.T) {
	root := newTestRootModel(t, "team-a")
	operations := &recordingClusterOperations{}
	root.browser.operations = operations
	command := root.drillDownCommand(DrillDownMsg{
		Screen:       ScreenBrowser,
		ResourceType: resourceTypeDeployments,
		ResourceName: "web",
	})
	if command == nil {
		t.Fatal("browser drill-down command = nil")
	}
	want := kube.ResourceReference{
		Resource:  schema.GroupResource{Group: apiGroupApps, Resource: resourceTypeDeployments},
		Namespace: "team-a",
	...
	if operations.inspected != want {
		t.Fatalf("drill-down reference = %+v, want %+v", operations.inspected, want)
	}

	message := root.drillDownCommand(DrillDownMsg{
		Screen:       ScreenBrowser,
		ResourceType: "widget",
		ResourceName: "sample",
	})().(cluster.DescribeMsg)
	if !errors.Is(message.Err, ErrUnsupportedResourceKind) {
		t.Fatalf("unsupported drill-down error = %v", message.Err)
	}
}
