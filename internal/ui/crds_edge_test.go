...
func TestCRDMouseWheelMovesUp(t *testing.T) {
	model := newTestCRDsModel("team-a")
	model.crds = make([]cluster.CRD, 10)
...
func TestCRDListCommandPreservesRequestScope(t *testing.T) {
	...
	if payload, ok := listMessage.payload.(cluster.CRDsMsg); !ok || payload.Err != nil {
		t.Fatalf("list payload = %#v", listMessage.payload)
	}
}
...
func TestCRDInstancesCommandPreservesRequestScope(t *testing.T) {
	...
	crd := cluster.CRD{Group: "example.com", Plural: "widgets", PreferredVersion: "v1", Resource: "widgets.example.com"}
	instanceMessage, ok := model.fetchCRDInstances(crd)().(crdResultMsg)
	...
	if payload, ok := instanceMessage.payload.(cluster.CRDInstancesMsg); !ok || payload.Err != nil {
		t.Fatalf("instance payload = %#v", instanceMessage.payload)
	}
}
...
func TestCRDAcceptsCurrentInstanceResultAndRejectsUnknownKind(t *testing.T) {
	model := newTestCRDsModel("team-a")
	model.view = crdsViewInstances
	model.selectedCRD = cluster.CRD{Resource: "widgets.example.com"}
...
func TestCRDBodiesRenderErrorsEmptyStatesAndTables(t *testing.T) {
	...
	model.err = nil
	model.instances = []cluster.CRDInstance{{Name: "widget", Namespace: "team-a"}}
	...
	model.err = nil
	model.crds = []cluster.CRD{{Name: "widgets.example.com"}}
	model.crdsTable.SetRows(model.crdsRows())
