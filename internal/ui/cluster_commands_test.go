...
type testKubeLiveSet[T any] struct {
	changes chan struct{}
	state   kube.LiveState[T]
	stop    sync.Once
}

func newTestKubeLiveSet[T any](items []T, err error) kube.LiveSet[T] {
...
func TestNativeClusterCommandsFetchEveryResource(t *testing.T) {
	...
	commands := newNativeClusterCommands(context.Background(), reader, &testResourceObserver{})
	...
	tests := []struct {
		...
		{name: "pods", command: func() any { return commands.FetchPods("ns")() }, want: cluster.PodsMsg{}},
		{name: "deployments", command: func() any { return commands.FetchDeployments("ns")() }, want: cluster.DeploymentsMsg{}},
		{name: "events", command: func() any { return commands.FetchEvents("ns")() }, want: cluster.EventsMsg{}},
		{name: "pod metrics", command: func() any { return commands.FetchPodMetrics("ns")() }, want: cluster.MetricsMsg{}},
		{name: "services", command: func() any { return commands.FetchServices("ns")() }, want: cluster.ServicesMsg{}},
		{name: "stateful sets", command: func() any { return commands.FetchStatefulSets("ns")() }, want: cluster.StatefulSetsMsg{}},
		{name: "daemon sets", command: func() any { return commands.FetchDaemonSets("ns")() }, want: cluster.DaemonSetsMsg{}},
		{name: "config maps", command: func() any { return commands.FetchConfigMaps("ns")() }, want: cluster.ConfigMapsMsg{}},
		{name: "nodes", command: func() any { return commands.FetchNodes()() }, want: cluster.NodesMsg{}},
		{name: "jobs", command: func() any { return commands.FetchJobs("ns")() }, want: cluster.JobsMsg{}},
		{name: "ingresses", command: func() any { return commands.FetchIngresses("ns")() }, want: cluster.IngressesMsg{}},
		{name: "network policies", command: func() any { return commands.FetchNetworkPolicies("ns")() }, want: cluster.NetworkPoliciesMsg{}},
		{name: "persistent volume claims", command: func() any { return commands.FetchPVCs("ns")() }, want: cluster.PVCsMsg{}},
		{name: "cron jobs", command: func() any { return commands.FetchCronJobs("ns")() }, want: cluster.CronJobsMsg{}},
		{name: "horizontal pod autoscalers", command: func() any { return commands.FetchHPAs("ns")() }, want: cluster.HPAsMsg{}},
		{name: "secrets", command: func() any { return commands.FetchSecrets("ns")() }, want: cluster.SecretsMsg{}},
		{name: "replica sets", command: func() any { return commands.FetchReplicaSets("ns")() }, want: cluster.ReplicaSetsMsg{}},
		{name: "role-based access control", command: func() any { return commands.FetchRBAC("ns")() }, want: cluster.RBACMsg{}},
		{name: "custom resource definitions", command: func() any { return commands.FetchCRDs()() }, want: cluster.CRDsMsg{}},
		{name: "custom resource instances", command: func() any {
			crd := cluster.CRD{Group: "example.invalid", Plural: "widgets", PreferredVersion: "v1", Resource: "widgets.example.invalid"}
			return commands.FetchCRDInstances(crd, "ns")()
		}, want: cluster.CRDInstancesMsg{}},
	}
...
func TestNativeClusterCommandsUseClusterScopeForClusterCRDs(t *testing.T) {
	...
	message := commands.FetchCRDInstances(crd, "selected-namespace")().(cluster.CRDInstancesMsg)
