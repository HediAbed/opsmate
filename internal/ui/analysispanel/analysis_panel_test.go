...
func TestAnalysisPanelViewDoesNotChangeRetainedWidgetGeometry(t *testing.T) {
	...
	viewportWidth, viewportHeight := model.responseView.Width(), model.responseView.Height()

	_ = model.View()
	if model.input.Width() != inputWidth {
		t.Fatalf("View changed input width from %d to %d", inputWidth, model.input.Width())
	}
	if model.responseView.Width() != viewportWidth || model.responseView.Height() != viewportHeight {
		t.Fatalf("View changed response geometry from %dx%d to %dx%d", viewportWidth, viewportHeight, model.responseView.Width(), model.responseView.Height())
	}
}
...
func TestHistory_UpdateInPlace(t *testing.T) {
	...
	m, _ = m.Update(analysis.AnalysisMsg{Response: response})
...
func TestRebuildChatContent_EmptyState(t *testing.T) {
	...
	if !strings.Contains(plain, "Cluster Analysis") {
		t.Error("empty state should show the cluster analysis heading")
	}
...
func TestRebuildChatContent_WithHistory(t *testing.T) {
	...
	if !strings.Contains(plain, "ANALYSIS") {
		t.Error("chat content should contain the analysis label")
	}
}
...
func TestAnalysisPanelAnalysisPayloadKeepsEvidenceOutOfSystemInstructions(t *testing.T) {
...
func TestAnalysisPanelRecentConversationMemory_BoundedToRecentTurns(t *testing.T) {
	m := NewAnalysisPanelModel()
...
func TestAnalysisPanelClearChat_ClearsHistoryAndInput(t *testing.T) {
	m := NewAnalysisPanelModel()
	m.SetSize(80, 30)
...
func TestStreamChunkMsg_Accumulates(t *testing.T) {
	...
	m.loading = true
	m.streaming = true
	m.streamChan = make(chan analysis.StreamEvent, 10)
	m.streamBuffer = ""

	m2, _ := m.Update(analysis.StreamChunkMsg{Chunk: "Hello "})
	if !m2.streaming {
...
func TestStreamChunkMsg_Done(t *testing.T) {
	...
	m2, _ := m.Update(analysis.StreamChunkMsg{Done: true})
	if m2.streaming {
...
func TestStreamChunkMsg_Error(t *testing.T) {
	...
	m2, _ := m.Update(analysis.StreamChunkMsg{Err: errors.New("stream failed")})
	if m2.streaming {
