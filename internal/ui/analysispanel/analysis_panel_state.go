...
import "github.com/HediAbed/opsmate/internal/terminal"
...
func (m *AnalysisPanelModel) SetContext(screenContext string) {
	m.response = terminal.SanitizeText(screenContext)
...
func (m *AnalysisPanelModel) SetScreenContext(ctx string) {
	m.screenContext = terminal.SanitizeText(ctx)
}

func (m *AnalysisPanelModel) RefreshProviderName() {
	m.providerName = terminal.SanitizeText(m.providerNameOf())
}
