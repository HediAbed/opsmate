...
const (
	...
	analysisPanelInputVerticalChrome       = 4
	analysisPanelHelpInsertionIndex        = 2
...
func NewAnalysisPanelModel() AnalysisPanelModel {
	return NewWithService(analysis.NewService(nil))
}

func NewWithService(service analysis.Service) AnalysisPanelModel {
	queryInput := component.NewTextInput(component.TextInputOptions{
		Prompt:      "> ",
	...
	responseView := component.NewViewport(analysisPanelInitialWidth, analysisPanelInitialViewportHeight)
	...
	return AnalysisPanelModel{
		...
		providerName:      terminal.SanitizeText(service.ProviderName()),
		...
		analyzeCluster:    clusterui.UnavailableAnalysis,
...
func (AnalysisPanelModel) Accepts(msg tea.Msg) bool {
	_, accepted := msg.(ResultMsg)
	return accepted
}

func (m *AnalysisPanelModel) SetClusterAnalyzer(analyzer func(string, string, string, string) tea.Cmd) {
	m.analyzeCluster = analyzer
}

func (m AnalysisPanelModel) ScreenContext() string {
	return m.screenContext
}

func (m AnalysisPanelModel) InputValue() string {
	return m.input.Value()
}

func (m AnalysisPanelModel) InputFocused() bool {
	return m.input.Focused()
}

func (m AnalysisPanelModel) Response() string {
	return m.response
}
...
func wrapAnalysisRequest(requestID uint64, command tea.Cmd) tea.Cmd {
	...
	return func() tea.Msg {
		return ResultMsg{RequestID: requestID, Payload: command()}
	}
}
...
func (m AnalysisPanelModel) handleAnalysisRequestResult(msg ResultMsg) (AnalysisPanelModel, tea.Cmd) {
...
func (m *AnalysisPanelModel) handleAnalysisResult(msg analysis.AnalysisMsg) tea.Cmd {
	...
	if msg.Err != nil {
		...
		m.err = nil
		m.response = terminal.SanitizeText(msg.Response)
...
func generatedCommandResponse(msg analysis.GeneratedCommandMsg) string {
	command := terminal.SanitizeText(msg.Command)
	explanation := terminal.SanitizeText(msg.Explanation)
...
func (m *AnalysisPanelModel) handleMouseClick(msg tea.MouseClickMsg) tea.Cmd {
	if msg.Y < m.innerHeight()-analysisPanelInputVerticalChrome {
		if m.input.Focused() {
			m.input.Blur()
		}
		return nil
...
func (m *AnalysisPanelModel) handleStreamError(err error) tea.Cmd {
	m.endStream()
	m.err = err
	m.setLatestResponseIfEmpty("Error: " + terminal.SanitizeText(err.Error()) + "\n\n" + retryHint)
	m.rebuildChatContent()
	m.input.Focus()
	return textinput.Blink
}
...
func (m *AnalysisPanelModel) finishStream() tea.Cmd {
	m.streamBuffer = terminal.SanitizeText(m.streamRaw)
...
func (m *AnalysisPanelModel) submitInput() tea.Cmd {
	...
	query := terminal.SanitizeText(strings.TrimSpace(m.input.Value()))
	if query == "" {
		return nil
	}
	if !m.hasProvider() {
