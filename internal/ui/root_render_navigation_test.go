...
func TestRootActiveScreenViewHandlesAnalysisAndInvalidScreen(t *testing.T) {
	model := freshRoot(t)
	model.screen = ScreenAnalysis
	model.analysisPanel.SetVisible(true)
	model.analysisPanel.SetSize(80, 20)
	if rendered := model.activeScreenView(); rendered == "" {
		t.Error("analysis screen rendered empty content")
	}
...
func TestRootAnalysisPanelOffsetsFollowScreenChrome(t *testing.T) {
	for _, screen := range []screenID{ScreenBrowser, ScreenLogs, ScreenHelm, ScreenCRDs, ScreenDashboard, ScreenAnalysis} {
		model := freshRoot(t)
		model.screen = screen
		top, bottom := model.analysisPanelOverlayOffsets(30)
...
func TestRootPaletteCoversPortForwardAndActiveBrowserPaths(t *testing.T) {
	...
	if !ok || !errors.Is(feedback.Err, ErrPortForwardNamespaceRequired) {
...
func TestRootPickerBoundaryStates(t *testing.T) {
	...
	model.contexts = []cluster.KubeContext{{Name: "current", Current: true}}
...
func TestRootPortForwardModalRendersUnselectedAndConfirmationRows(t *testing.T) {
	model := freshRoot(t)
	first := testModelPortForwardSession(t, "one", "api", 8080, 80)
	first.StartedAt = time.Now()
	second := testModelPortForwardSession(t, "two", "worker", 9090, 90)
	second.StartedAt = time.Now()
	model.pfSessions = []kube.PortForwardSession{first, second}
	model.pfCursor = 1
...
func TestRootResizeAndAnalysisPanelHeightBoundaryPaths(t *testing.T) {
	...
	model = freshRoot(t)
	for _, screen := range []screenID{ScreenBrowser, ScreenLogs, ScreenHelm, ScreenCRDs, ScreenDashboard, ScreenAnalysis, screenID(255)} {
		model.screen = screen
		height := model.analysisPanelHeight(30)
		if height <= 0 || height > 30 {
			t.Errorf("screen %d analysis panel height = %d", screen, height)
		}
	}
}

func TestRootContextErrorsClearAnalysisEvidence(t *testing.T) {
	model := freshRoot(t)
	...
	if code := failure.CodeOf(model.err); code != failure.CodeInternal {
		t.Fatalf("invalid screen failure code = %q, want %q", code, failure.CodeInternal)
	}
