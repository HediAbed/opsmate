...
func projectPods(items []corev1.Pod, now time.Time) []model.Pod {
	return projectSlice(items, now, projectPod)
}

func projectPod(item corev1.Pod, now time.Time) model.Pod {
	ready := 0
	restarts := 0
...
func projectDeployment(item appsv1.Deployment, now time.Time) model.Deployment {
...
func projectEvents(items []corev1.Event, now time.Time) []model.Event {
	events := projectSlice(items, now, projectEvent)
	slices.SortStableFunc(events, func(left, right model.Event) int {
...
func projectEvent(item corev1.Event, now time.Time) model.Event {
...
func projectPodMetrics(items []metricsv1beta1.PodMetrics, _ time.Time) []model.PodMetric {
	metrics := make([]model.PodMetric, 0, len(items))
	for _, item := range items {
		cpu, memory := podMetricUsage(item)
		metrics = append(metrics, model.PodMetric{
			Name:      item.Name,
			Namespace: item.Namespace,
			CPU:       cpu,
			Memory:    memory,
...
func projectService(item corev1.Service, now time.Time) model.Service {
