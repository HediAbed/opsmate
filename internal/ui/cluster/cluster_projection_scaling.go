...
func projectCronJobs(items []batchv1.CronJob, now time.Time) []model.CronJob {
	return projectSlice(items, now, func(item batchv1.CronJob, now time.Time) model.CronJob {
...
func projectHPAs(items []autoscalingv2.HorizontalPodAutoscaler, now time.Time) []model.HPA {
	return projectSlice(items, now, projectHPA)
}

func projectHPA(item autoscalingv2.HorizontalPodAutoscaler, now time.Time) model.HPA {
	...
	return model.HPA{
		Name:      item.Name,
		Namespace: item.Namespace,
		Reference: model.ScaleTargetRef{
...
func projectedHPATargets(specs []autoscalingv2.MetricSpec, statuses []autoscalingv2.MetricStatus) []model.HPAMetricPair {
	if len(specs) == 0 {
		return []model.HPAMetricPair{{Target: projectedNone}}
	}
	statusByKey := make(map[string]autoscalingv2.MetricStatus, len(statuses))
	for _, status := range statuses {
		statusByKey[metricStatusKey(status)] = status
	}
	pairs := make([]model.HPAMetricPair, 0, len(specs))
	for _, spec := range specs {
		current := projectedUnknown
		if status, found := statusByKey[metricSpecKey(spec)]; found {
			current = projectedMetricStatus(status)
		}
		pairs = append(pairs, model.HPAMetricPair{
			Current: current,
