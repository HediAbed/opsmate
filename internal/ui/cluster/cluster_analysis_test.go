...
func TestNativeClusterAnalyzerCollectsAndAnalyzesSnapshot(t *testing.T) {
	...
	analyzer := NewAnalyzer(context.Background(), collector, analysis.NewService(nil))
	var received struct {
		systemPrompt       string
		question           string
...
func TestNativeClusterAnalyzerRejectsMissingDependencies(t *testing.T) {
	...
	service := analysis.NewService(nil)
	tests := []struct {
		name     string
		analyzer SnapshotAnalyzer
	}{
		{name: "parent", analyzer: SnapshotAnalyzer{snapshots: collector, analyze: service.AnalyzeClusterContext}},
		{name: "collector", analyzer: SnapshotAnalyzer{parent: context.Background(), analyze: service.AnalyzeClusterContext}},
		{name: "analysis", analyzer: SnapshotAnalyzer{parent: context.Background(), snapshots: collector}},
	}
...
func TestUnavailableClusterAnalysisReturnsTypedMessage(t *testing.T) {
	message := UnavailableAnalysis("system", "question", "", "")().(analysis.AnalysisMsg)
	if !errors.Is(message.Err, ErrClusterAnalysisUnavailable) {
		t.Fatalf("UnavailableAnalysis() error = %v", message.Err)
	}
}
