...
type CRDsModel struct {
	...
	crds        []clustermodel.CRD
	crdsTable   table.Model
	selectedCRD clustermodel.CRD

	instances     []clustermodel.CRDInstance
...
var crdsListColSpecs = []component.ColumnSpec{
	{Title: "NAME", Flex: component.ColumnFlexHalf, Min: component.ColumnMinimumName},
	{Title: "GROUP", Flex: component.ColumnFlexQuarter, Min: component.ColumnMinimumReadable},
	{Title: "SCOPE", Width: component.ColumnWidthMedium},
	{Title: "VERSIONS", Flex: component.ColumnFlexSmall, Min: component.ColumnMinimumCompact},
	{Title: "AGE", Width: component.ColumnWidthCompact},
}

var crdInstanceColSpecs = []component.ColumnSpec{
	{Title: "NAME", Flex: component.ColumnFlexPrimary, Min: component.ColumnMinimumName},
	{Title: "NAMESPACE", Flex: component.ColumnFlexQuarter, Min: component.ColumnMinimumStandard},
	{Title: "AGE", Width: component.ColumnWidthCompact},
}

const initialTableWidth = 80

func NewCRDsModel(namespace string, commands clusterui.Commands) CRDsModel {
	...
	return CRDsModel{
		...
		cluster:       commands,
		view:          crdsViewList,
		crdsTable:     component.NewTable(initialTableWidth, crdsListColSpecs),
		instanceTable: component.NewTable(initialTableWidth, crdInstanceColSpecs),
...
func (m *CRDsModel) SetSize(width, height int) {
	...
	m.crdsTable.SetColumns(component.Columns(width, crdsListColSpecs))
	m.instanceTable.SetWidth(width)
	m.instanceTable.SetColumns(component.Columns(width, crdInstanceColSpecs))
}
...
func (m CRDsModel) Size() (int, int) {
	return m.width, m.height
}

func (m CRDsModel) Loading() bool {
	return m.loading
}

func (CRDsModel) Accepts(msg tea.Msg) bool {
	_, accepted := msg.(crdResultMsg)
	return accepted
}
...
func (m CRDsModel) Update(msg tea.Msg) (CRDsModel, tea.Cmd) {
	...
	case clustermodel.CRDsMsg:
		m.applyCRDsResult(msg)
		return m, nil

	case clustermodel.CRDInstancesMsg:
...
func (m *CRDsModel) applyCRDsResult(msg clustermodel.CRDsMsg) {
	m.loading = false
	if msg.Err != nil {
		m.err = msg.Err
		m.crds = nil
	...
	m.statusMsg = theme.Success.Render(fmt.Sprintf("Loaded %d %s", len(msg.CRDs), component.NounForCount("crd", "crds", len(msg.CRDs))))
}

func (m CRDsModel) applyCRDInstancesResult(msg clustermodel.CRDInstancesMsg) CRDsModel {
	...
	m.statusMsg = theme.Success.Render(fmt.Sprintf("Loaded %d %s of %s", len(msg.Instances), component.NounForCount("instance", "instances", len(msg.Instances)), m.selectedCRD.Kind))
	return m
}
...
func (m *CRDsModel) handleCRDMouseWheel(msg tea.MouseWheelMsg) {
	...
	case tea.MouseWheelUp:
		activeTable.MoveUp(component.TableWheelStep)
	case tea.MouseWheelDown:
		activeTable.MoveDown(component.TableWheelStep)
	}
}
...
func (m CRDsModel) backToList() (CRDsModel, tea.Cmd) {
	...
	m.view = crdsViewList
	m.selectedCRD = clustermodel.CRD{}
	m.instances = nil
	m.instanceTable.SetRows(nil)
	m.statusMsg = ""
	m.instanceRequestID++
...
func (m *CRDsModel) fetchCRDInstances(crd clustermodel.CRD) tea.Cmd {
	m.instanceRequestID++
