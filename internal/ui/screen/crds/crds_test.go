...
func TestCRDsModel_CRDsMsg_PopulatesListView(t *testing.T) {
	m := newTestCRDsModel("ns")
	m.SetSize(160, 40)

	out, _ := m.Update(cluster.CRDsMsg{CRDs: []cluster.CRD{
...
func TestCRDsModel_EscReturnsToListView(t *testing.T) {
	...
	m.selectedCRD = cluster.CRD{Name: "x.example.com", Resource: "x.example.com"}
	m.instances = []cluster.CRDInstance{{Name: "i"}}
...
func TestCRDsModel_StaleInstancesMsg_DroppedAfterCRDSwapped(t *testing.T) {
	...
	m.SetSize(160, 40)
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Resource: "current.example.com"}
	m.loading = true

	stale := cluster.CRDInstancesMsg{
		Resource:  "old.example.com",
		Namespace: "ns",
		Instances: []cluster.CRDInstance{{Name: "stale"}},
	}
...
func TestCRDsModel_StaleInstancesMsg_DroppedAfterNamespaceSwapped(t *testing.T) {
	m := newTestCRDsModel("ns-current")
	m.SetSize(160, 40)
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Resource: "x.example.com"}
	m.loading = true
...
func TestCRDsModel_InstancesMsgPopulatesTable(t *testing.T) {
	...
	m.SetSize(160, 40)
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Resource: "x.example.com", Kind: "X"}

	out, _ := m.Update(cluster.CRDInstancesMsg{
		Resource:  "x.example.com",
		Namespace: "ns",
...
func TestCRDsModel_View_EmptyInstancesShowsScopedMessage(t *testing.T) {
	...
	m.SetSize(160, 40)
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Kind: "Certificate"}
	m.loading = false
...
func TestCRDsModel_SelectedCRDName_FromCursorOrDrilldown(t *testing.T) {
	...
	m.crds = []cluster.CRD{{Name: "a.example.com"}, {Name: "b.example.com"}}
	m.crdsTable.SetRows(m.crdsRows())
	...
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Name: "drilled.example.com"}
...
func TestCRDsModel_CRDsMsg_ErrorClearsRowsAndSetsBanner(t *testing.T) {
	m := newTestCRDsModel("ns")
	m.SetSize(160, 40)
	m.crds = []cluster.CRD{{Name: "stale.example.com"}}
	m.crdsTable.SetRows(m.crdsRows())
	m.loading = true

	out, _ := m.Update(cluster.CRDsMsg{Err: errStub("boom")})
	if out.err == nil {
...
func TestCRDsModel_CRDInstancesMsg_ErrorClearsInstancesAndSetsBanner(t *testing.T) {
	...
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Resource: "x.example.com"}
	m.instances = []cluster.CRDInstance{{Name: "stale"}}
	m.instanceTable.SetRows(m.instanceRows())
	m.loading = true

	out, _ := m.Update(cluster.CRDInstancesMsg{Resource: "x.example.com", Namespace: "ns", Err: errStub("boom")})
	if out.err == nil {
...
func TestCRDsHandleKey_DefaultForwardsToInstanceTable(_ *testing.T) {
	...
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Resource: "x.example.com"}
	m.instances = []cluster.CRDInstance{{Name: "a"}, {Name: "b"}}
	m.instanceTable.SetRows(m.instanceRows())
...
func TestCRDsModel_AnalysisOverlayBounds_AccountsForChromeAndClampsToMin(t *testing.T) {
	m := newTestCRDsModel("ns")
	m.SetSize(160, 40)
...
func TestCRDsModel_MouseClick_RoutesToInstanceTableInInstanceView(t *testing.T) {
	...
	m.SetSize(120, 30)
	m.view = crdsViewInstances
	m.selectedCRD = cluster.CRD{Name: "x.io", Resource: "x"}
	m.instances = []cluster.CRDInstance{
...
func TestCRDsModel_MouseClick_NonLeftButtonIgnored(t *testing.T) {
	m := newTestCRDsModel("ns")
	m.SetSize(120, 30)
	m.crds = []cluster.CRD{{Name: "a.io"}, {Name: "b.io"}}
	m.crdsTable.SetRows(m.crdsRows())
	clickY := m.tableFirstRowY() + 1
...
func TestCRDsModel_MouseWheel_MovesActiveTableCursor(t *testing.T) {
	m := newTestCRDsModel("ns")
	m.SetSize(120, 30)
	m.crds = make([]cluster.CRD, 10)
	for i := range m.crds {
		m.crds[i] = cluster.CRD{Name: "x"}
	}
	m.crdsTable.SetRows(m.crdsRows())
...
func TestCRDsModel_SetSize_PreservesCursors(t *testing.T) {
	m := newTestCRDsModel("ns")
	m.SetSize(120, 30)
	m.crds = []cluster.CRD{{Name: "a"}, {Name: "b"}, {Name: "c"}}
	m.crdsTable.SetRows(m.crdsRows())
	m.crdsTable.SetCursor(2)

	m.instances = []cluster.CRDInstance{{Name: "x"}, {Name: "y"}}
	m.instanceTable.SetRows(m.instanceRows())
