...
func TestHelmModel_AnalysisOverlayBounds_AccountsForChromeAndClampsToMin(t *testing.T) {
	...
	if smallPanel < component.MinimumAnalysisPanelHeight {
		t.Errorf("panelHeight should clamp to %d; got %d", component.MinimumAnalysisPanelHeight, smallPanel)
	}
}
...
func TestHelmModel_MouseWheel_MovesCursor(t *testing.T) {
	...
	if got := out.releaseTable.Cursor(); got != component.TableWheelStep {
		t.Errorf("wheel down should advance cursor by %d; got %d", component.TableWheelStep, got)
	}
