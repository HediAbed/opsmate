...
func TestHelmValuesViewUsesPrecomputedViewportGeometry(t *testing.T) {
	...
	model.valuesPopupRelease = "api"
	model.syncValuesPopupLayout()
	width, height := model.valuesPopupView.Width(), model.valuesPopupView.Height()

	_ = model.View()
	if model.valuesPopupView.Width() != width || model.valuesPopupView.Height() != height {
		t.Fatalf("View changed values geometry from %dx%d to %dx%d", width, height, model.valuesPopupView.Width(), model.valuesPopupView.Height())
	}
}
