...
func dashPodColumns(innerW int) []table.Column {
	cellPad := dashboardPodColumnCount * component.TableCellPadding
...
func (m *DashboardModel) rebuildTableRows() {
	...
	identities := make([]podIdentity, 0, len(m.pods))
	for _, pod := range m.pods {
		...
		rows = append(rows, table.Row{
			displayPodName(pod.Namespace, pod.Name, m.namespace == ""),
			pod.Status,
			pod.Ready,
		...
		identities = append(identities, podIdentity{Namespace: pod.Namespace, Name: pod.Name})
	}
