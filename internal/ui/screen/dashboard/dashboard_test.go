...
func TestDashboardSearchItemsIncludePodsAndDeployments(t *testing.T) {
	m := newTestDashboardModel("default")
	m.pods = []cluster.Pod{{Name: "api", Namespace: "backend"}}
	m.deployments = []cluster.Deployment{{Name: "worker", Namespace: "jobs"}}

	items := m.SearchItems()
	if len(items) != 2 {
		t.Fatalf("search items = %d; want 2", len(items))
	}
	if items[0].Kind != screen.ResourceKindPod || items[0].Name != "api" || items[0].Namespace != "backend" {
		t.Errorf("pod search item = %+v", items[0])
	}
	if items[1].Kind != screen.ResourceKindDeployment || items[1].Name != "worker" || items[1].Namespace != "jobs" {
		t.Errorf("deployment search item = %+v", items[1])
	}
}
