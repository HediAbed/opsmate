...
func TestApplyLogsStopsRefreshWhilePaused(t *testing.T) {
	model := newLogsForTest(t)
	model.paused = true
	command := model.applyLogs(cluster.LogsMsg{Lines: []string{"line"}})
...
func TestApplyContainersHandlesPodWithoutContainers(t *testing.T) {
	model := newLogsForTest(t)
	if command := model.applyContainers(cluster.ContainersMsg{}); command != nil || len(model.containers) != 0 {
...
func TestLogMouseWheelRoutesPopupAndDisablesFollowOnUp(t *testing.T) {
	model := newLogsForTest(t)
	model.pods = []cluster.Pod{{Name: "one"}, {Name: "two"}}
	model.showPodPopup = true
...
func TestLogsViewRendersOptionalSectionsAndPopupModes(t *testing.T) {
	model := newLogsForTest(t)
	model.inspectMode = true
	model.lineExplanation = "explanation"
	...
	model.showPodPopup = true
	model.pods = []cluster.Pod{{Name: "web"}}
...
func newLogsWithFailingOperations(t *testing.T) LogsModel {
	t.Helper()
	operations := &testOperations{logErr: errors.New("log read failed")}
	return NewLogsModel("team-a", testCommands{}, operations)
}
...
func TestLogPodCommandPreservesRequestScope(t *testing.T) {
	...
	if payload, ok := podsMessage.payload.(cluster.PodsMsg); !ok || payload.Err != nil {
		t.Fatalf("pod payload = %#v", podsMessage.payload)
	}
}

func TestSelectedLogsCommandPreservesRequestScope(t *testing.T) {
	model := newLogsWithFailingOperations(t)
	...
	if payload, ok := logsMessage.payload.(cluster.LogsMsg); !ok || payload.Err == nil {
...
func TestPodPopupClickIgnoresChromeRows(t *testing.T) {
	...
	popupLeft := (model.width - popupWidth) / 2
	popupTop := (model.height - popupHeight) / 2
...
func TestLogExplanationAndIssueNavigationHandleMissingTargets(t *testing.T) {
	...
	model.lineExplanationLoading = true
	if command := model.explainInspectedLine(); command != nil {
...
func TestPodPopupRendersEmptySmallScrolledAndCurrentStates(t *testing.T) {
	...
	for index := range 8 {
		model.pods = append(model.pods, cluster.Pod{
