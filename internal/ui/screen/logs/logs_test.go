...
func TestLogsSpinnerTickIdleProducesNoFollowUp(t *testing.T) {
	model := newTestLogsModel("default")
	model.loading = false
	model.lineExplanationLoading = false

	_, command := model.Update(spinner.TickMsg{})
	if command != nil {
		t.Error("idle logs scheduled another spinner tick")
	}
}

func TestLogsSoftWrapEnabled(t *testing.T) {
	model := newTestLogsModel("ns")
	if !model.logView.SoftWrap {
		t.Error("logs viewport does not wrap long lines")
	}
}
...
func TestLogsLayoutSupportsScrollingBeforeView(t *testing.T) {
	...
	model.selectedPodNamespace = "payments"
	lines := make([]string, 100)
	for index := range lines {
		lines[index] = fmt.Sprintf("line %d", index)
	}
	model, _ = model.Update(cluster.LogsMsg{Lines: lines})
	model.logView.GotoTop()

	if model.logView.Width() <= 0 || model.logView.Height() <= 0 {
		t.Fatalf("log viewport was not sized: %dx%d", model.logView.Width(), model.logView.Height())
	}
	model, _ = model.Update(tea.MouseWheelMsg{Button: tea.MouseWheelDown})
	if model.logView.YOffset() == 0 {
		t.Fatal("log viewport did not scroll before View")
	}
}

func TestSelectedPodNS_SingleNamespace(t *testing.T) {
	m := newTestLogsModel("production")
...
func TestNewLogsModel_Defaults(t *testing.T) {
	m := newTestLogsModel("kube-system")
...
func TestNewLogsModel_InspectDefaults(t *testing.T) {
	...
	if m.lineExplanation != "" {
		t.Errorf("aiExplanation should be empty by default, got %q", m.lineExplanation)
	}
	if m.lineCursor != 0 {
