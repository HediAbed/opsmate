...
func TestLogsModel_HandlePopupKey_UpDownNavigate(t *testing.T) {
	m := newLogsForTest(t)
	m.pods = []cluster.Pod{{Name: "a"}, {Name: "b"}, {Name: "c"}}
	m.podCursor = 0
...
func TestLogsModel_RenderExplainPanel_ShowsExplanationOrLoadingOrError(t *testing.T) {
	...
	m.lineExplanation = "this means the pod ran out of memory"
	...
	m.lineExplanationErr = errStub("boom")
	if stripAnsiForTest(m.renderExplainPanel()) == "" {
...
func TestLogsModel_RenderPodPopupOverlay_NotEmpty(t *testing.T) {
	m := newLogsForTest(t)
	m.pods = []cluster.Pod{{Name: "alpha"}, {Name: "beta"}}
	m.podCursor = 0
...
func TestLogsModel_HandleInspectKey_EnterRequestsExplanation(t *testing.T) {
	m := newLogsForTest(t)
	m.inspectMode = true
	...
	if cmd == nil {
		t.Error("enter should return ExplainLogLine cmd")
	}
}
