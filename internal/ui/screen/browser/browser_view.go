...
func (m BrowserModel) renderTitleBar() string {
	...
	if tabsWidth < browserTabStripMinimumWidth {
		tabsWidth = browserTabStripMinimumWidth
	}
...
func renderBrowserTabStrip(active string, maxWidth int) string {
	if maxWidth < browserTabStripMinimumViable {
		return ""
	}
...
func (m BrowserModel) renderSplitContent(height int) string {
	...
	detailTitle := theme.Accent.Render(" Detail ") + "  " +
		...
		"  " + component.PopupScrollIndicator(m.detailView)
...
func (m BrowserModel) renderConfirmBox() string {
	...
	return lipgloss.NewStyle().
		...
		Width(component.FitModalWidth(confirmationModalDesiredWidth, m.width)).
		MaxWidth(m.width).
...
func (m BrowserModel) lookupReplicaInfoFor(identity resourceIdentity) string {
	...
	case resourceTypeDeployments:
		return m.deploymentReplicaInfo(identity)
	case resourceTypeStatefulSets:
		return m.statefulSetReplicaInfo(identity)
	}
	return ""
}

func (m BrowserModel) deploymentReplicaInfo(identity resourceIdentity) string {
...
func (m BrowserModel) statefulSetReplicaInfo(identity resourceIdentity) string {
...
func formatEventsOutput(events []cluster.Event) string {
	...
	for _, ev := range events {
		eventType := terminal.SanitizeText(ev.Type)
		reason := terminal.SanitizeText(ev.Reason)
		object := terminal.SanitizeText(ev.Object)
		msg := terminal.SanitizeText(ev.Message)
