...
func TestBrowser_ToggleWide_FlipsPositionalColumnCount(t *testing.T) {
	m := newTestBrowserModel("default")
	m.resourceType = "pods"
	m.pods = []cluster.Pod{{Name: "p", Status: "Running", Ready: "1/1", Age: "1m", IP: "10.0.0.1", Node: "n"}}
	normalCols := len(resourceColSpecs["pods"])
	wideCols := len(resourceColSpecsWide["pods"])
	if normalCols == wideCols {
		t.Fatal("wide and normal pods specs must differ in width; fixture is invalid")
	}
