...
func TestBrowserRoutesEveryTypedLiveResource(t *testing.T) {
	assertBrowserLiveSnapshot(t, "deployments", screen.LiveSnapshot[cluster.Deployment]{State: clusterui.LiveState[cluster.Deployment]{Ready: true, Items: []cluster.Deployment{{Name: "web", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.deployments) })
	assertBrowserLiveSnapshot(t, "ingresses", screen.LiveSnapshot[cluster.Ingress]{State: clusterui.LiveState[cluster.Ingress]{Ready: true, Items: []cluster.Ingress{{Name: "web", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.ingresses) })
	assertBrowserLiveSnapshot(t, "networkpolicies", screen.LiveSnapshot[cluster.NetworkPolicy]{State: clusterui.LiveState[cluster.NetworkPolicy]{Ready: true, Items: []cluster.NetworkPolicy{{Name: "default-deny", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.networkpolicies) })
	assertBrowserLiveSnapshot(t, "pvcs", screen.LiveSnapshot[cluster.PersistentVolumeClaim]{State: clusterui.LiveState[cluster.PersistentVolumeClaim]{Ready: true, Items: []cluster.PersistentVolumeClaim{{Name: "data", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.pvcs) })
	assertBrowserLiveSnapshot(t, "cronjobs", screen.LiveSnapshot[cluster.CronJob]{State: clusterui.LiveState[cluster.CronJob]{Ready: true, Items: []cluster.CronJob{{Name: "cleanup", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.cronjobs) })
	assertBrowserLiveSnapshot(t, "hpas", screen.LiveSnapshot[cluster.HPA]{State: clusterui.LiveState[cluster.HPA]{Ready: true, Items: []cluster.HPA{{Name: "web", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.hpas) })
	assertBrowserLiveSnapshot(t, "secrets", screen.LiveSnapshot[cluster.Secret]{State: clusterui.LiveState[cluster.Secret]{Ready: true, Items: []cluster.Secret{{Name: "credentials", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.secrets) })
	assertBrowserLiveSnapshot(t, "replicasets", screen.LiveSnapshot[cluster.ReplicaSet]{State: clusterui.LiveState[cluster.ReplicaSet]{Ready: true, Items: []cluster.ReplicaSet{{Name: "web-123", Namespace: "team-a"}}}}, func(model BrowserModel) int { return len(model.replicasets) })
}
...
func TestBrowserResultAndInputRoutingCoverTransientMessages(t *testing.T) {
	...
	model, command := model.updateBrowserResultMessage(screen.ClearStatusMsg{})
...
func TestClearStatusAfterProducesClearMessage(t *testing.T) {
	message := screen.ClearStatusAfter(0)()
	if _, ok := message.(screen.ClearStatusMsg); !ok {
