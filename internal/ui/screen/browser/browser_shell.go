...
func (m BrowserModel) openShell() (BrowserModel, tea.Cmd) {
	...
	if m.resourceType != resourceTypePods {
		...
		return m, screen.ClearStatusAfter(shellUnavailableStatusDuration)
	}
...
func newShellViewport(width, height int) viewport.Model {
	...
	return component.NewViewport(viewportWidth, viewportHeight)
}
...
func (m BrowserModel) interruptShell() (BrowserModel, tea.Cmd) {
...
func (m BrowserModel) submitShellCommand() (BrowserModel, tea.Cmd) {
	...
	if err := m.shellSession.Send(cmd); err != nil {
		m.appendShellLine(theme.Error.Render("error: " + terminal.SanitizeLine(err.Error())))
	}
	return m, nil
}
...
func (m BrowserModel) handleShellOutput(msg shellOutputMsg) (BrowserModel, tea.Cmd) {
	...
	rendered := terminal.SanitizeText(msg.Line)
	if msg.Stderr {
