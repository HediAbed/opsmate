...
func TestRootCollectSearchCorpus_AggregatesAcrossSubmodels(t *testing.T) {
	m := freshRoot(t)
	m.dashboard.pods = []service.Pod{{Name: "dash-pod", Namespace: "ns1"}}
	m.dashboard.deployments = []service.Deployment{{Name: "dash-dep", Namespace: "ns1"}}
	m.browser.pods = []service.Pod{{Name: "br-pod", Namespace: "ns2"}}
	m.browser.deployments = []service.Deployment{{Name: "br-dep", Namespace: "ns2"}}
	m.browser.services = []service.Service{{Name: "br-svc", Namespace: "ns2"}}
	m.browser.statefulsets = []service.StatefulSet{{Name: "br-sts", Namespace: "ns2"}}
	m.browser.daemonsets = []service.DaemonSet{{Name: "br-ds", Namespace: "ns2"}}
	m.browser.configmaps = []service.ConfigMap{{Name: "br-cm", Namespace: "ns2"}}
	m.browser.jobs = []service.Job{{Name: "br-job", Namespace: "ns2"}}
	m.logs.pods = []service.Pod{{Name: "logs-pod", Namespace: "ns3"}}
...
func TestRootCollectSearchCorpus_DedupesIdenticalEntries(t *testing.T) {
	m := freshRoot(t)
	m.dashboard.pods = []service.Pod{{Name: "shared", Namespace: "ns"}}
	m.browser.pods = []service.Pod{{Name: "shared", Namespace: "ns"}}
	got := m.collectSearchCorpus()
	count := 0
	for _, r := range got {
		if r.Name == "shared" {
