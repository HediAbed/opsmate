...
func TestDashboard_RenderDeploymentHealth_MaxSix(t *testing.T) {
	...
	m.SetSize(120, 40)
	for i := 0; i < 10; i++ {
		m.deployments = append(m.deployments, service.Deployment{
...
func TestDashboard_AIHealth_Defaults(t *testing.T) {
	m := NewDashboardModel("default")
	if m.showAIHealth {
		t.Error("showAIHealth should be false by default")
	}
	if m.aiHealthLoading {
		t.Error("aiHealthLoading should be false by default")
	}
	if m.aiHealthSummary != "" {
		t.Errorf("aiHealthSummary should be empty by default, got %q", m.aiHealthSummary)
	}
}

func TestDashboard_RenderAIHealth_Loading(t *testing.T) {
	m := NewDashboardModel("default")
	m.SetSize(120, 40)
	m.showAIHealth = true
	m.aiHealthLoading = true
	got := m.renderAIHealth(m.innerW())
	if got == "" {
		t.Error("renderAIHealth should not be empty when loading")
	}
	plain := stripANSI(got)
	if !strings.Contains(plain, "Analyzing") {
		t.Errorf("renderAIHealth loading should contain 'Analyzing', got: %q", plain)
	}
}

func TestDashboard_RenderAIHealth_WithSummary(t *testing.T) {
	m := NewDashboardModel("default")
	m.SetSize(120, 40)
	m.showAIHealth = true
	m.aiHealthSummary = "Cluster is healthy. All pods running."
	got := m.renderAIHealth(m.innerW())
	plain := stripANSI(got)
	if !strings.Contains(plain, "CLUSTER HEALTH") {
		t.Errorf("renderAIHealth should contain header, got: %q", plain)
	}
	if !strings.Contains(plain, "Cluster is healthy") {
		t.Errorf("renderAIHealth should contain summary text, got: %q", plain)
	}
}

func TestDashboard_RenderAIHealth_Hidden(t *testing.T) {
	m := NewDashboardModel("default")
	m.SetSize(120, 40)
	...
	plain := stripANSI(got)
	if strings.Contains(plain, "CLUSTER HEALTH") {
		t.Error("View should not contain AI health when showAIHealth is false")
	}
}
