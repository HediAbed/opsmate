...
func TestHelmValuesResponseUpdatesMatchingPopup(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "values",
			message:    service.HelmValuesMsg{Release: "gateway", Namespace: "edge", Values: "replicas: 3"},
			wantText:   "replicas: 3",
			wantLoaded: true,
		},
		{
			name:       "chart defaults",
			message:    service.HelmValuesMsg{Release: "gateway", Namespace: "edge", Values: "  \n"},
			wantText:   "chart defaults",
			wantLoaded: true,
		},
		{
			name:       "request error",
			message:    service.HelmValuesMsg{Release: "gateway", Namespace: "edge", Err: errors.New("access denied\x1b[31m")},
...
func TestHelmValuesResponseRejectsInactiveOrDifferentPopup(t *testing.T) {
	message := service.HelmValuesMsg{Release: "gateway", Namespace: "edge", Values: "new"}
	inactive := NewHelmModel("edge")
...
func TestHelmValuesResultEnvelopeRejectsStaleAndAcceptsCurrent(t *testing.T) {
	...
	current := helmResultMsg{
		...
		namespace: "edge",
		release:   "gateway",
		payload:   service.HelmValuesMsg{Release: "gateway", Namespace: "edge", Values: "enabled: true"},
	}
	stale := current
	stale.requestID--
...
func installFakeHelmResponses(t *testing.T) {
	t.Helper()
	binDirectory := t.TempDir()
	writeTestExecutable(t, filepath.Join(binDirectory, "helm"), `#!/bin/sh
case "$1" in
list)
  printf '[{"name":"gateway","namespace":"edge","revision":"2","status":"deployed"}]\n'
  ;;
get)
  printf 'replicas: 3\n'
  ;;
...
func TestHelmFetchReleasesWrapsSuccessfulResponse(t *testing.T) {
	installFakeHelmResponses(t)

	model := NewHelmModel("edge")
	releasesEnvelope := model.fetchReleases()().(helmResultMsg)
	releases, ok := releasesEnvelope.payload.(service.HelmReleasesMsg)
...
func TestHelmFetchValuesWrapsSuccessfulResponse(t *testing.T) {
	...
	values, ok := valuesEnvelope.payload.(service.HelmValuesMsg)
...
func TestHelmLayoutAndFallbackRenderingPaths(t *testing.T) {
	...
	model.releases = []service.HelmRelease{{Name: "different", Namespace: "edge"}}
...
func helmModelWithSelectedRelease() HelmModel {
	model := NewHelmModel("edge")
	model.SetSize(120, 40)
	model.releases = []service.HelmRelease{{Name: "gateway", Namespace: "edge", Revision: 2}}
