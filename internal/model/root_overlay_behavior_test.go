...
func TestRootPrimaryOverlaysOwnKeyboardInput(t *testing.T) {
	...
	t.Run("port forwards", func(t *testing.T) {
		...
		model.pfSessions = []service.PortForwardSession{{ID: "one"}, {ID: "two"}}
...
func TestRootSecondaryOverlaysOwnKeyboardInput(t *testing.T) {
	t.Run("context picker", func(t *testing.T) {
		...
		model.contexts = []service.KubeContext{{Name: "one"}, {Name: "two"}}
		updated, _ := model.handleRootKey(tea.KeyPressMsg{Code: tea.KeyDown})
	...
	t.Run("assistant input", func(t *testing.T) {
		model := freshRoot(t)
		model.aiPanel.SetVisible(true)
		model.aiPanel.Focus()
		updated, _ := model.handleRootKey(tea.KeyPressMsg{Code: 'x', Text: "x"})
		if value := updated.(RootModel).aiPanel.input.Value(); value != "x" {
			t.Errorf("assistant input = %q", value)
		}
	})

	t.Run("full assistant closes to dashboard", func(t *testing.T) {
		model := freshRoot(t)
		model.screen = ScreenAI
		model.aiPanel.SetVisible(true)
		updated, _ := model.handleRootKey(tea.KeyPressMsg{Code: tea.KeyEsc})
		root := updated.(RootModel)
		if root.screen != ScreenDashboard || root.aiPanel.IsVisible() {
			t.Errorf("escape left screen=%d visible=%v", root.screen, root.aiPanel.IsVisible())
		}
	})
}
...
func TestRootFocusedScreenInterruptPolicy(t *testing.T) {
	...
	if command != nil || updated.(RootModel).browser.state != stateShell {
...
func TestRootAssistantToggleCanCloseVisiblePanel(t *testing.T) {
	model := freshRoot(t)
	model.aiPanel.SetVisible(true)
	model.toggleAssistantPanel()
	if model.aiPanel.IsVisible() {
		t.Error("assistant panel remained visible")
	}
}
...
func TestRootUpdateRoutesMouseAndUnknownMessages(t *testing.T) {
	model := freshRoot(t)
	model.dashboard.pods = []service.Pod{{Name: "first"}, {Name: "second"}}
...
func TestRootAssistantMouseRejectsMainPane(t *testing.T) {
	model := freshRoot(t)
	model.aiPanel.SetVisible(true)
	updated, command, handled := model.handleAssistantMouse(tea.MouseClickMsg{X: 1, Y: 1, Button: tea.MouseLeft})
	if handled || command != nil || updated.(RootModel).screen != model.screen {
		t.Fatalf("main-pane mouse claimed by assistant: handled=%v command=%v", handled, command)
	}
}
