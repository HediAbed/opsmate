...
func TestSetResourceType_WatchUpdateUsesCurrentTableShape(t *testing.T) {
	...
	m.deployments = []service.Deployment{{Name: "dep", Namespace: "default", Ready: "1/1", Age: "1d"}}
	m.rebuildTable()

	m.SetResourceType("pods")

	ev := service.WatchEventMsg[service.Pod]{
	...
	_ = handleTypedWatchEvent(&m, &m.podWatcher, &m.pods, podKey, "pods", ev)
	...
	if rows := m.resourceTable.Rows(); len(rows) != 1 || len(rows[0]) != len(resourceColSpecs["pods"]) {
		t.Fatalf("pod watch produced incompatible rows: %v", rows)
	}
