...
func TestHandleBrowsingKey_CCopiesRow(t *testing.T) {
	...
	m.SetSize(200, 40)
	m.SetResourceType("pods")
	m.pods = []service.Pod{{Name: "alpha", Namespace: "ns", Status: "Running"}}
	m.rebuildTable()
...
func TestHandleBrowsingKey_SOnDeploymentEntersScaleInput(t *testing.T) {
	...
	m.SetSize(200, 40)
	m.SetResourceType("deployments")
	m.deployments = []service.Deployment{{Name: "web", Ready: "1/1", UpToDate: 1}}
	m.rebuildTable()
...
func TestHandleScaleInputState_EnterValidNumberShowsConfirm(t *testing.T) {
	...
	m.SetSize(200, 40)
	m.SetResourceType("deployments")
	m.deployments = []service.Deployment{{Name: "web"}}
	m.rebuildTable()
	m.state = stateScaleInput
...
func TestHandleDetailState_AKeyTriggersAIAnalysis(t *testing.T) {
	m := NewBrowserModel("ns")
	m.SetSize(200, 40)
	m.state = stateDetail
	m.showDetail = true
	...
	if cmd == nil {
		t.Error("a should return AI cmd")
	}
}
...
func TestHandleDetailState_AWhileLoadingIsNoOp(t *testing.T) {
	...
	_ = out
	if cmd != nil {
		t.Error("a while AI loading must be a no-op")
	}
}
