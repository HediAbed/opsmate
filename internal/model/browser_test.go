...
func TestFormatEventsOutput_Empty(t *testing.T) {
	...
	got = formatEventsOutput([]service.Event{})
	if got != "No events found." {
...
func TestBrowser_AISummary_Defaults(t *testing.T) {
	m := NewBrowserModel("default")
	if m.aiSummary != "" {
		t.Errorf("aiSummary should be empty by default, got %q", m.aiSummary)
	}
	if m.aiSummaryLoad {
		t.Error("aiSummaryLoad should be false by default")
	}
	if m.aiSummaryErr != nil {
		t.Errorf("aiSummaryErr should be nil by default, got %v", m.aiSummaryErr)
	}
}
...
func TestBrowser_RebuildDetailContent_WithSummary(t *testing.T) {
	...
	m.SetSize(120, 40)
	m.detailContent = "Name: test-pod\nNamespace: default"
	m.aiSummary = "Pod is running normally with no issues."
	m.rebuildDetailContent()
	got := m.detailView.View()
	if !strings.Contains(got, "running normally") {
		t.Error("rebuildDetailContent should contain AI summary text")
	}
}
