...
func TestWrapAIRequestHandlesNilAndPreservesRequestIdentity(t *testing.T) {
	if wrapAIRequest(1, nil) != nil {
		t.Fatal("nil command produced a wrapper")
	}

	command := wrapAIRequest(42, func() tea.Msg { return "result" })
	message, ok := command().(aiRequestResultMsg)
	if !ok || message.requestID != 42 || message.payload != "result" {
		t.Fatalf("wrapped message = %#v", message)
	}
}

func TestAIPanelInputRoutingHandlesUnknownMessagesAndFocusedClick(t *testing.T) {
	panel := NewAIPanelModel()
	panel.SetSize(80, 30)
	panel.input.Focus()
...
func TestAIPanelClearKeysResetActiveConversation(t *testing.T) {
...
func TestAIPanelSetLatestResponseIgnoresEmptyHistory(t *testing.T) {
	panel := NewAIPanelModel()
...
func TestAIPanelViewRendersProviderAndActivityStates(t *testing.T) {
	states := []struct {
		name      string
		configure func(*AIPanelModel)
		want      string
	}{
		{name: "provider", configure: func(panel *AIPanelModel) { panel.providerName = "configured" }, want: "[configured]"},
		{name: "streaming", configure: func(panel *AIPanelModel) { panel.streaming = true }, want: "streaming"},
		{name: "loading", configure: func(panel *AIPanelModel) { panel.loading = true }, want: "thinking"},
		{name: "confirmation", configure: func(panel *AIPanelModel) {
			panel.showConfirm = true
			panel.pendingCommand = "kubectl get pods"
		}, want: "execute?"},
	}
...
func TestRiskPaletteFallsBackForInvalidRisk(t *testing.T) {
	border, headline := riskPalette(service.CommandRisk(255))
	if border != theme.Yellow || headline != theme.Yellow {
		t.Fatalf("fallback palette = (%v, %v), want yellow", border, headline)
	}
}
