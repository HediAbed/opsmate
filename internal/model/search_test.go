...
func TestCollectSearchCorpus_DedupsAcrossSources(t *testing.T) {
	...
	shared := service.Pod{Name: "web-7d9f", Namespace: "default"}
	m.dashboard.pods = []service.Pod{shared}
	m.browser.pods = []service.Pod{shared}
...
func TestCollectSearchCorpus_SkipsNamelessEntries(t *testing.T) {
	m := NewRootModel("default")
	m.browser.pods = []service.Pod{{Name: "", Namespace: "default"}}
...
func TestCollectSearchCorpus_CoversAllResourceKinds(t *testing.T) {
	m := NewRootModel("default")
	m.browser.pods = []service.Pod{{Name: "p", Namespace: "ns"}}
	m.browser.deployments = []service.Deployment{{Name: "d", Namespace: "ns"}}
	m.browser.services = []service.Service{{Name: "s", Namespace: "ns"}}
	m.browser.statefulsets = []service.StatefulSet{{Name: "ss", Namespace: "ns"}}
	m.browser.daemonsets = []service.DaemonSet{{Name: "ds", Namespace: "ns"}}
	m.browser.configmaps = []service.ConfigMap{{Name: "cm", Namespace: "ns"}}
	m.browser.jobs = []service.Job{{Name: "j", Namespace: "ns"}}
...
func TestOpenSearch_PopulatesCorpusOnce(t *testing.T) {
	...
	m.browser.pods = append(m.browser.pods, service.Pod{Name: "second", Namespace: "ns"})
	got := m.filterSearchResults("")
	if len(got) != 1 {
