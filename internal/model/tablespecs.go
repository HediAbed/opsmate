...
const (
	...
	// separator (1) — the lines bubbles' table renders before data rows.
...
// resourceColSpecsWide adds the `kubectl get -o wide` extras for the
// resource types where the extra fields pay off the most. Resource types
// absent from this map fall back to resourceColSpecs in wide mode.
...
func buildResourceTable(width int, specs []colSpec) table.Model {
	...
	t.SetStyles(browserTableStyles())
	return t
}
...
func shrinkFixedToFit(cols []table.Column, specs []colSpec, available int) {
	...
	for i := range cols {
		...
		w := cols[i].Width * available / total
		if w < 1 {
			w = 1
		}
		cols[i].Width = w
...
func flexWidth(remaining, weight, totalWeight, minimum int) int {
	w := 0
	if totalWeight > 0 {
		w = remaining * weight / totalWeight
	}
	if w < minimum {
		w = minimum
	}
