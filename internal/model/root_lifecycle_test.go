...
func TestRootModel_RestoreSession_AppliesPersistedScreen(t *testing.T) {
	m := NewRootModel("ns")
	m.RestoreSession(service.SessionState{Screen: int(ScreenBrowser), ResourceType: "pods"})
...
func TestRootModel_RestoreSession_RejectsOutOfRange(t *testing.T) {
	...
	m.RestoreSession(service.SessionState{Screen: 99})
...
func TestRootModel_DeactivateScreen_ClearsScreenActivity(t *testing.T) {
	...
	if m.dashboard.active || m.browser.active || m.logs.active || m.aiPanel.IsVisible() || m.helm.loading || m.crds.loading {
		t.Fatalf("deactivation left active state: dashboard=%v browser=%v logs=%v assistant=%v helm=%v crds=%v",
			m.dashboard.active, m.browser.active, m.logs.active, m.aiPanel.IsVisible(), m.helm.loading, m.crds.loading)
	}
}
...
func TestRootModel_RenderContent_CtxPickerOverlay(t *testing.T) {
	...
	m.contexts = []service.KubeContext{{Name: "ctx1"}}
	out := m.renderContent()
	if out == "" {
...
func TestRootModel_UpdateAIScreenContext_UsesActiveScreenData(t *testing.T) {
	m := freshRoot(t)
	m.dashboard.pods = []service.Pod{{Name: "dashboard-api", Namespace: "default"}}
	m.screen = ScreenDashboard
	m.updateAIScreenContext()
	if !strings.Contains(m.aiPanel.screenContext, "dashboard-api") {
		t.Fatalf("dashboard context = %q", m.aiPanel.screenContext)
	}

	m.browser.pods = []service.Pod{{Name: "browser-api", Namespace: "default"}}
	m.screen = ScreenBrowser
	m.updateAIScreenContext()
	if !strings.Contains(m.aiPanel.screenContext, "browser-api") {
		t.Fatalf("browser context = %q", m.aiPanel.screenContext)
	}
	...
	m.screen = ScreenLogs
	m.updateAIScreenContext()
	if !strings.Contains(m.aiPanel.screenContext, "logs-api") {
		t.Fatalf("logs context = %q", m.aiPanel.screenContext)
	}
}
