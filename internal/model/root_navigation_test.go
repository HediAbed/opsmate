...
func TestRootModel_Update_TabTogglesAIPanelOn(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.Update(tea.KeyPressMsg{Code: tea.KeyTab, Text: "tab"})
	r := model.(RootModel)
	if !r.aiPanel.IsVisible() {
		t.Error("tab should toggle AI panel on")
	}
}
...
func TestRootModel_Update_AIVisibleEscHidesIt(t *testing.T) {
	m := freshRoot(t)
	m.aiPanel.SetVisible(true)
	model, _ := m.Update(tea.KeyPressMsg{Code: tea.KeyEsc, Text: "esc"})
	r := model.(RootModel)
	if r.aiPanel.IsVisible() {
		t.Error("esc should hide AI panel")
	}
}
...
func TestRootUpdate_NamespacesMsg_PopulatesOrErr(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.Update(service.NamespacesMsg{Namespaces: []string{"default"}})
	r := model.(RootModel)
	if len(r.namespaces) != 1 {
		t.Errorf("namespaces should populate; got %d", len(r.namespaces))
	}

	model, _ = m.Update(service.NamespacesMsg{Err: errStub("denied")})
	r = model.(RootModel)
	if r.err == nil {
...
func TestRootUpdate_CurrentContextMsg(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.Update(service.CurrentContextMsg{Name: "my-cluster"})
	r := model.(RootModel)
...
func TestRootUpdate_ContextsMsg_PicksCurrentMarker(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.Update(service.ContextsMsg{Contexts: []service.KubeContext{
		{Name: "a"},
		{Name: "b", Current: true},
	}})
	r := model.(RootModel)
...
func TestRootUpdate_PortForwardStartedMsg_ReshresPFList(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.Update(service.PortForwardStartedMsg{Err: errStub("port in use")})
	r := model.(RootModel)
	if r.err == nil {
		t.Error("err should propagate")
	}
...
func TestRootUpdate_PortForwardStoppedMsg_ResetsPFCursor(t *testing.T) {
	m := freshRoot(t)
	m.pfCursor = 99
	model, _ := m.Update(service.PortForwardStoppedMsg{ID: "x"})
	_ = model
}
...
func TestRootUpdate_ContextSwitchedMsg_ResetsState(t *testing.T) {
	m := freshRoot(t)
	model, cmd := m.Update(service.ContextSwitchedMsg{Name: "new-ctx"})
	r := model.(RootModel)
...
func TestRootUpdate_ContextSwitchedMsg_ErrorPath(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.Update(service.ContextSwitchedMsg{Err: errStub("denied")})
	r := model.(RootModel)
	if r.err == nil {
		t.Error("err should propagate")
	}
...
func TestRootHandleDrillDown_DefaultJustSwitchesScreen(t *testing.T) {
	m := freshRoot(t)
	model, _ := m.handleDrillDown(DrillDownMsg{Screen: ScreenAI})
	r := model.(RootModel)
	if r.screen != ScreenAI {
		t.Errorf("screen = %d, want AI", r.screen)
	}
}
...
func TestRootHandleMouse_CtxPickerInterceptsAllEvents(t *testing.T) {
	...
	m.contexts = []service.KubeContext{{Name: "a"}}
	model, cmd := m.handleMouse(tea.MouseClickMsg{X: 5, Y: 5, Button: tea.MouseLeft})
	r := model.(RootModel)
...
func TestRootHandleMouse_AIVisibleAdjustsXForRightSideClick(t *testing.T) {
	m := freshRoot(t)
	m.aiPanel.SetVisible(true)
	m.resizeChildren()
	model, _ := m.handleMouse(tea.MouseClickMsg{X: m.width - 5, Y: 5, Button: tea.MouseLeft})
	r := model.(RootModel)
	if r.aiPanel.input.Focused() {
		t.Fatal("clicking the response area should blur the assistant input")
	}
}
...
func TestRootHandleMouse_LeftClickOnContentForwardsToScreen(t *testing.T) {
	...
	m.dashboard.pods = []service.Pod{{Name: "first"}, {Name: "second"}}
...
func TestRootHandlePFModalKey_DownNavigates(t *testing.T) {
	m := freshRoot(t)
	m.showPFModal = true
	m.pfSessions = []service.PortForwardSession{{ID: "a"}, {ID: "b"}}
	m.pfCursor = 0
...
func TestRootHandlePFModalKey_XSetsConfirmKill(t *testing.T) {
	m := freshRoot(t)
	m.showPFModal = true
	m.pfSessions = []service.PortForwardSession{{ID: "abc", Pod: "alpha", LocalPort: 8080, RemotePort: 80}}
	m.pfCursor = 0
...
func TestRootRenderPFModal_WithSessionsHighlightsCursor(t *testing.T) {
	m := freshRoot(t)
	m.pfSessions = []service.PortForwardSession{{ID: "a", Pod: "alpha", LocalPort: 8080, RemotePort: 80, Status: "running"}}
	out := stripAnsiForTest(m.renderPFModal(20))
	if out == "" {
...
func TestRootSaveSession_NoPanicOnNoConfigDir(t *testing.T) {
	t.Setenv("XDG_CONFIG_HOME", "")
	t.Setenv("HOME", "")
	m := freshRoot(t)
	if err := m.saveSession(); err == nil {
		t.Fatal("saveSession succeeded without a user config directory")
	}
}
