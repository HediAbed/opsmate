...
func TestApplyFetchResult_HappyPathPopulatesCacheAndClearsBanners(t *testing.T) {
	...
	pods := []service.Pod{{Name: "a", Namespace: "ns"}, {Name: "b", Namespace: "ns"}}
...
func TestApplyFetchResult_DispatchedKindAppearsInStatus(t *testing.T) {
	m := NewBrowserModel("ns")
	m.SetSize(120, 40)
	applyTypedFetchResult(&m, "cronjobs", &m.cronjobs, []service.CronJob{{Name: "n"}, {Name: "n2"}}, nil)
...
func TestApplyFetchResult_SingleItemUsesSingularNoun(t *testing.T) {
	m := NewBrowserModel("ns")
	m.SetSize(120, 40)
	applyTypedFetchResult(&m, "pods", &m.pods, []service.Pod{{Name: "only", Namespace: "ns"}}, nil)
...
func TestDisplayKind_PluralPassesThroughForCountOtherThanOne(t *testing.T) {
	cases := []struct {
		kind  string
		count int
		want  string
	...
	for _, c := range cases {
		if got := displayKind(c.kind, c.count); got != c.want {
			t.Errorf("displayKind(%q, %d) = %q; want %q", c.kind, c.count, got, c.want)
		}
	}
}

func TestDisplayKind_SingleCountUsesSingularFormPerKind(t *testing.T) {
	cases := []struct {
		kind, want string
	}{
		{"pods", "pod"},
	...
	for _, c := range cases {
		if got := displayKind(c.kind, 1); got != c.want {
			t.Errorf("displayKind(%q, 1) = %q; want %q", c.kind, got, c.want)
		}
	}
}
