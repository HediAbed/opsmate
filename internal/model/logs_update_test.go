...
func TestLogsUpdate_PodsMsg_PicksFirstPod(t *testing.T) {
	m := NewLogsModel("ns")
	m.SetSize(120, 30)
	out, cmd := m.Update(service.PodsMsg{Pods: []service.Pod{{Name: "alpha", Namespace: "ns"}}})
...
func TestLogsUpdate_LogsMsg_HappyPath(t *testing.T) {
	...
	m.SetSize(120, 30)
	m.selectedPod = "alpha"
	out, _ := m.Update(service.LogsMsg{Lines: []string{"line 1", "line 2"}})
	if out.loading {
...
func TestLogsUpdate_LogsMsg_Error(t *testing.T) {
	m := NewLogsModel("ns")
	out, cmd := m.Update(service.LogsMsg{Err: errStub("boom")})
	if out.err == nil {
...
func TestLogsUpdate_LogExplainMsg_Success(t *testing.T) {
	m := NewLogsModel("ns")
	m.aiExplainLoading = true
	out, _ := m.Update(service.LogExplainMsg{Explanation: "this is the OOM line"})
...
func TestLogsUpdate_LogExplainMsg_Error(t *testing.T) {
	m := NewLogsModel("ns")
	m.aiExplainLoading = true
	out, _ := m.Update(service.LogExplainMsg{Err: errStub("rate limited")})
	if out.aiExplainErr == nil {
...
func TestLogsUpdate_ContainersMsg_MultiplePopsContainerPopup(t *testing.T) {
	...
	out, _ := m.Update(service.ContainersMsg{Containers: []string{"main", "sidecar"}})
...
func TestLogsUpdate_ContainersMsg_SingleContainerSetsStatus(t *testing.T) {
	m := NewLogsModel("ns")
	m.SetSize(120, 30)
	out, _ := m.Update(service.ContainersMsg{Containers: []string{"main"}})
	if out.statusMsg == "" {
...
func TestLogsUpdate_ContainersMsg_Error(t *testing.T) {
	m := NewLogsModel("ns")
	out, _ := m.Update(service.ContainersMsg{Err: errStub("denied")})
	if out.statusMsg == "" {
