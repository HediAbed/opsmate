...
func TestBrowserConfirmedSingleActionsPreserveSelectedIdentity(t *testing.T) {
	tests := []struct {
		...
		{name: "delete", action: "delete", status: "Deleting pod/worker"},
		{name: "restart", action: "restart", status: "Restarting deployment/worker"},
	}
...
func TestBrowserMouseRoutesByInteractionState(t *testing.T) {
	...
	model.pods = append(model.pods, service.Pod{Name: "second", Namespace: "team-a", Status: "Running"})
	model.rebuildTable()
	updated, _ = model.handleBrowserMouseWheel(tea.MouseWheelMsg{Button: tea.MouseWheelDown})
