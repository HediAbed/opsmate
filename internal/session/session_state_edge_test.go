...
func TestSessionPathRejectsRelativeConfigDirectory(t *testing.T) {
	useRelativeTestConfigDirectory(t)
	if _, err := sessionPathFromConfigDirectory("relative"); !errors.Is(err, ErrUnsafeSession) {
		t.Fatalf("relative configuration directory error = %v, want ErrUnsafeSession", err)
	}
	if _, err := sessionPath(); err == nil {
		t.Fatal("resolved relative configuration directory did not fail")
	}
...
func TestLoadSessionDistinguishesMissingFileAndUnsafeDirectory(t *testing.T) {
	...
	if err := os.MkdirAll(sessionDirectory, sessionDirectoryMode); err != nil {
	...
	otherConfigDirectory := useTestConfigDirectory(t)
	if err := os.MkdirAll(filepath.Dir(otherConfigDirectory), sessionDirectoryMode); err != nil {
		t.Fatalf("create config parent: %v", err)
	}
	unsafePath := filepath.Join(otherConfigDirectory, "opsmate")
	if err := os.MkdirAll(otherConfigDirectory, sessionDirectoryMode); err != nil {
...
func TestReadBoundedSessionPropagatesReaderFailure(t *testing.T) {
	...
	if _, err := readBoundedSession(iotest.ErrReader(sentinel)); !errors.Is(err, sentinel) {
...
func TestSaveSessionReportsDirectoryPreparationFailure(t *testing.T) {
	configDirectory := useTestConfigDirectory(t)
	if err := os.MkdirAll(filepath.Dir(configDirectory), sessionDirectoryMode); err != nil {
		t.Fatalf("create config parent: %v", err)
	}
	if err := os.WriteFile(configDirectory, []byte("file"), sessionFileMode); err != nil {
...
func assertSessionOperationError(t *testing.T, err error, operation Operation, cause error) {
	t.Helper()
	var sessionErr *SessionError
