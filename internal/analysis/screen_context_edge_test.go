...
func TestDashboardContextStopsAtResourceAndEventBounds(t *testing.T) {
	...
	input := DashboardContextInput{
		Pods:        []cluster.Pod{{Name: longName}, {Name: longName}},
		Deployments: []cluster.Deployment{{Name: longName}},
		Events:      []cluster.Event{{Object: longName}, {Object: longName}},
	}
...
func TestBrowserRowsStopWhenContextIsFull(t *testing.T) {
	...
	writeRows(writer, []cluster.Pod{{Name: "pod"}, {Name: "second"}}, podContextRow)
	if !writer.Full() {
