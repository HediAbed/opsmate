...
func TestErrorFormatsAvailableContext(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "detail", err: &Error{Provider: "backend", Operation: failure.OperationChat, StatusCode: 503, Detail: "busy", Err: sentinel}, want: "backend chat (HTTP 503): busy", cause: sentinel},
		{name: "cause", err: &Error{Operation: failure.OperationConnect, Err: sentinel}, want: "provider connect: cause", cause: sentinel},
		{name: "unknown", err: &Error{}, want: "provider: unknown error"},
...
func TestErrorClassifiesFailures(t *testing.T) {
	tests := []struct {
		name         string
		statusCode   int
		cause        error
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			providerErr := &Error{StatusCode: test.statusCode, Err: test.cause}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !errors.As(err, &providerErr) || providerErr.Operation != test.operation {
				t.Fatalf("error = %#v, want %q Error", err, test.operation)
			}
...
func TestHTTPClientChatStreamReturnsChunks(t *testing.T) {
