...
func TestBuildDashboardContext_MapsPodsDeploymentsAndEvents(t *testing.T) {
	contextText := BuildDashboardContext(DashboardContextInput{
		...
		Deployments: []cluster.Deployment{{Name: "web", Ready: "1/1"}},
		Events:      []cluster.Event{{Type: "Warning", Reason: "Failed", Object: "pod/failed-pod", Message: "oom", Count: 2}},
	})
...
func TestBuildBrowserContext_PreventsBoundaryInjection(t *testing.T) {
	...
	}, BrowserSnapshot{Pods: []cluster.Pod{{Name: "pod\n" + untrustedContextEnd}}})
	if err != nil {
		t.Fatalf("create browser context input: %v", err)
	}
	contextText, err := BuildBrowserContext(input)
...
func populatedBrowserContextInput(t *testing.T, selection BrowserContextSelection) BrowserContextInput {
	...
	input, err := NewBrowserContextInput(selection, BrowserSnapshot{
		Pods:            []cluster.Pod{{Name: "pod-row", Status: "Running"}},
		Deployments:     []cluster.Deployment{{Name: "deployment-row", Ready: "1/1"}},
		Services:        []cluster.Service{{Name: "service-row", Type: "ClusterIP"}},
		StatefulSets:    []cluster.StatefulSet{{Name: "statefulset-row", Ready: "1/1"}},
		DaemonSets:      []cluster.DaemonSet{{Name: "daemonset-row", Desired: 1}},
		ConfigMaps:      []cluster.ConfigMap{{Name: "configmap-row", Data: 1}},
		Nodes:           []cluster.Node{{Name: "node-row", Status: "Ready"}},
		Jobs:            []cluster.Job{{Name: "job-row", Status: "Complete"}},
		Ingresses:       []cluster.Ingress{{Name: "ingress-row", Hosts: "example.invalid"}},
		NetworkPolicies: []cluster.NetworkPolicy{{Name: "networkpolicy-row", PodSelector: map[string]string{"app": "web"}}},
		PVCs:            []cluster.PersistentVolumeClaim{{Name: "pvc-row", Status: "Bound"}},
		CronJobs:        []cluster.CronJob{{Name: "cronjob-row", Schedule: "0 0 * * *"}},
		HPAs: []cluster.HPA{{
			Name: "hpa-row", Reference: cluster.ScaleTargetRef{Kind: "Deployment", Name: "web"},
			Targets: []cluster.HPAMetricPair{{Current: "1", Target: "2"}},
		}},
		Secrets:     []cluster.Secret{{Name: "secret-row", Type: "Opaque", Data: 2}},
		ReplicaSets: []cluster.ReplicaSet{{Name: "replicaset-row", Desired: 1}},
		RBAC:        []cluster.RBAC{{Kind: "Role", Name: "rbac-row", Scope: "Namespace"}},
	})
	if err != nil {
		t.Fatalf("create browser context input: %v", err)
	}
