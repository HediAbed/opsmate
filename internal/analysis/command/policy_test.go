...
func TestErrorFormatsAndUnwraps(t *testing.T) {
	...
	tests := []struct {
		...
		{err: &Error{Detail: "blocked", Err: sentinel}, want: "kubectl policy: blocked"},
		{err: &Error{Err: sentinel}, want: "kubectl policy: failed"},
		{err: &Error{}, want: "kubectl policy: unknown error"},
	}
	...
	if !errors.Is(&Error{Err: sentinel}, sentinel) {
...
func TestScopeKubectlCommand_AddsActiveNamespace(t *testing.T) {
	command, err := Scope("kubectl get pods", "operations")
	if err != nil {
...
func TestScopeKubectlCommand_PreservesMatchingNamespace(t *testing.T) {
	...
	got, err := Scope(command, "operations")
	if err != nil {
		t.Fatalf("Scope: %v", err)
	}
	if got != command {
...
func TestScopeKubectlCommand_RejectsScopeChanges(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := Scope(test.command, test.namespace)
			if !errors.Is(err, ErrCommandScope) || !errors.Is(err, ErrForbiddenCommand) {
				t.Fatalf("Scope error = %v, want command-scope policy error", err)
			}
		})
	}
}
...
func TestScopeKubectlCommand_QuotesNamespaceAsOneArgument(t *testing.T) {
	...
	command, err := Scope("kubectl get pods", namespace)
	if err != nil {
		t.Fatalf("Scope: %v", err)
	}
	args, err := validateKubectl(command)
