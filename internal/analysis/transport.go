...
func (e *ProviderError) FailureCode() failure.Code {
	if e == nil {
...
func providerTimeoutError(provider string, operation failure.Operation, timeout time.Duration) error {
	return &ProviderError{
		Provider:  provider,
		Operation: operation,
...
func executeChatCompletionWithEncoder(
	...
	if err != nil {
		return "", &ProviderError{Provider: provider, Operation: failure.OperationEncode, Err: err}
	}
	...
	if len(response.Choices) == 0 {
		return "", &ProviderError{Provider: provider, Operation: failure.OperationChat, Err: ErrProviderEmptyResponse}
	}
