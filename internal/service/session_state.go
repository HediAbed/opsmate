...
var (
	...
	ErrUnsafeSession   = errors.New("saved session path is not a regular file")
)
...
func sessionPath() (string, error) {
	...
	if err != nil {
		return "", &SessionError{Operation: "locate config directory", Err: err}
	}
...
func loadSessionAtPath(path string, openRoot sessionRootOpener) (_ SessionState, returnErr error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return SessionState{}, &SessionError{Operation: "load", Path: path, Err: ErrNoSession}
		}
		return SessionState{}, &SessionError{Operation: "open directory", Path: directory, Err: err}
	}
	defer func() {
	...
	if err != nil {
		...
		return SessionState{}, &SessionError{Operation: "open", Path: path, Err: err}
	}
	...
	if err := json.Unmarshal(data, &state); err != nil {
		return SessionState{}, &SessionError{Operation: "decode", Path: path, Err: err}
	}
	return state, nil
}
...
func saveSessionAtPath(
	...
	if err := prepareDirectory(directory); err != nil {
		return &SessionError{Operation: "prepare directory", Path: directory, Err: err}
	}
	root, err := openRoot(directory)
	if err != nil {
		return &SessionError{Operation: "open directory", Path: directory, Err: err}
	}
	defer func() {
		if closeErr := root.Close(); closeErr != nil {
			returnErr = errors.Join(returnErr, &SessionError{
	...
	if err := validateSessionDestination(root, sessionFileName); err != nil {
		return &SessionError{Operation: "validate destination", Path: path, Err: err}
	}
	data, err := encode(state)
	if err != nil {
		return &SessionError{Operation: "encode", Path: path, Err: err}
	}
	temporaryName, err := writeSessionTemp(root, data)
	if err != nil {
		return &SessionError{Operation: "write temporary state", Path: directory, Err: err}
	}
	if err := replaceSessionFile(root, temporaryName, sessionFileName); err != nil {
		...
		return &SessionError{Operation: "replace", Path: path, Err: errors.Join(err, cleanupErr)}
	}
	return nil
}
