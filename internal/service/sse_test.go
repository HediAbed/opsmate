...
func TestExtractGeminiChunk(t *testing.T) {
	valid := []byte(`{"candidates":[{"content":{"parts":[{"text":"hello"}]}}]}`)
	text, err := extractGeminiChunk(valid)
	if err != nil || text != "hello" {
		t.Fatalf("valid payload = (%q, %v), want hello", text, err)
	}
	text, err = extractGeminiChunk([]byte(`{"candidates":[]}`))
	if err != nil || text != "" {
		t.Fatalf("empty payload = (%q, %v), want empty without error", text, err)
	}
	text, err = extractGeminiChunk([]byte(`{"candidates":[{"content":{"parts":[]}}]}`))
	if err != nil || text != "" {
		t.Fatalf("empty parts = (%q, %v), want empty without error", text, err)
	}
	_, err = extractGeminiChunk([]byte(`{bad}`))
	if err == nil {
		t.Fatal("malformed payload must fail")
	}
	var providerErr *ProviderError
	if !errors.As(err, &providerErr) || providerErr.Provider != "Gemini" {
		t.Fatalf("error = %#v, want Gemini ProviderError", err)
	}
}

func TestExtractOpenAICompatibleChunk(t *testing.T) {
	valid := []byte(`{"choices":[{"delta":{"content":"hello"}}]}`)
	text, err := extractKimiChunk(valid)
	if err != nil || text != "hello" {
		t.Fatalf("valid payload = (%q, %v), want hello", text, err)
	}
	text, err = extractKimiChunk([]byte(`{"choices":[]}`))
	if err != nil || text != "" {
		t.Fatalf("empty payload = (%q, %v), want empty without error", text, err)
	}
	_, err = extractKimiChunk([]byte(`not json`))
	if err == nil {
		t.Fatal("malformed payload must fail")
	}
}
